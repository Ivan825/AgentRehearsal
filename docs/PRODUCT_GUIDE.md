...
| --- | --- |
| 0 Define | The agent under test: name, purpose, system prompt, session facts, the context line the agent sees. Tools: name, description, parameters, simulated responses keyed by argument. Rules in plain English → constraints (Bedrock parses, you confirm) → Cedar policy (shown). Scenarios: 13 seeds, add your own, generate more with Bedrock, delete any. Save / Load SupportBot example / Import JSON / Export JSON. |
